...
class HLTPrescaleExample : public edm::one::EDAnalyzer<edm::one::WatchLuminosityBlocks,edm::one::WatchRuns> {
public:
  HLTPrescaleExample(edm::ParameterSet const& iPSet);
  
  void beginJob() override {} 
  void beginRun(edm::Run const& iEvent, edm::EventSetup const&) override;
  void beginLuminosityBlock(edm::LuminosityBlock const& iEvent, edm::EventSetup const&) override {}
...
void HLTPrescaleExample::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup)
{
  ...
  std::cout <<"hltDouble "<<hltPSDouble<<" hltInt "<<hltPSInt<<" hltFrac "<<hltPSFrac<<std::endl;

 
  std::cout <<"l1HLTDouble "<<l1HLTPSDouble.first<<" "<<l1HLTPSDouble.second
	    <<" l1HLTInt "<<l1HLTPSInt.first<<" "<<l1HLTPSInt.second 
	    <<" l1HLTFrac "<<l1HLTPSFrac.first<<" "<<l1HLTPSFrac.second << std::endl;
