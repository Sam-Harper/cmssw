#include "CommonTools/MVAUtils/interface/GBRForestToolsTinyXML.h"
...
createGBRForestTinyXML(const std::string &weightsFile)
{
    std::vector<std::string> varNames;
    return createGBRForestTinyXML(weightsFile, varNames);
}

std::unique_ptr<const GBRForest>
createGBRForestTinyXML(const edm::FileInPath &weightsFile)
{
    std::vector<std::string> varNames;
    return createGBRForestTinyXML(weightsFile.fullPath(), varNames);
}

// Overloaded versions which are taking string vectors by reference to store the variable names in
std::unique_ptr<const GBRForest>
createGBRForestTinyXML(const std::string &weightsFile, std::vector<std::string> &varNames)
{
...
std::unique_ptr<const GBRForest>
createGBRForestTinyXML(const edm::FileInPath &weightsFile, std::vector<std::string> &varNames)
{
    return createGBRForestTinyXML(weightsFile.fullPath(), varNames);
}
