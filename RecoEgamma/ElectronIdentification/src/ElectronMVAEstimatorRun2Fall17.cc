...
mvaValue( const int iCategory, const std::vector<float> & vars) const  {
  ...
  if(debug) {
    ...
    std::cout << " bin "                      << iCategory << std::endl
              << " see "                      << vars[0] << std::endl
              << " spp "                      << vars[1] << std::endl
              << " OneMinusE1x5E5x5 "         << vars[2] << std::endl
              << " R9 "                       << vars[3] << std::endl
...
fillMVAVariables(const reco::GsfElectron* eleRecoPtr, const edm::Handle<reco::ConversionCollection> conversions, const reco::BeamSpot *theBeamSpot, const edm::Handle<double> rho) const {
  ...
  if(withIso)
  {
    ...
    std::vector<float> vars = std::move( packMVAVariables(
                                             ...
                                             // Endcap only variables
                                             // NOTE: we don't need to check if
                                             // we are actually in the endcap
                                             // or not, as it is the last
                                             // variable in the vector and it
                                             // will be ignored by the
                                             // GBRForest for the barrel.
                                             // The GBRForest classification
                                             // just needs an array with the
                                             // input variables in the right
                                             // order, what comes after doesn't
                                             // matter.
                                             PreShowerOverRaw          // 24
                                        )
                        );
