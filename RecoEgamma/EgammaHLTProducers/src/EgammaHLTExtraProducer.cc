...
void EgammaHLTExtraProducer::produce(edm::StreamID streamID,
  ...
  for (const auto& egCandsToken : tokens_.egCands) {
    auto ecalCandsHandle = getHandle(event,egCandsToken.first.ecalCands);
    auto gsfTrksHandle = getHandle(event,egCandsToken.first.gsfTracks);
    auto pixelSeedsHandle = getHandle(event,egCandsToken.first.pixelSeeds);
  ...
  auto filterAndStoreRecHits = [caloGeomHandle, &event, this](const auto& egTrigObjs, const auto& tokenLabels) {
    for (const auto& tokenLabel : tokenLabels) {
      auto handle = getHandle(event,tokenLabel.first);
