...
ElectronEnergyCalibrator::ElectronEnergyCalibrator(EpCombinationToolSemi &combinator,
						   bool isMC,
...
std::vector<float> ElectronEnergyCalibrator::calibrate(reco::GsfElectron &ele,
...
std::vector<float> ElectronEnergyCalibrator::calibrate(reco::GsfElectron &ele, unsigned int runNumber, 
						       const EcalRecHitCollection *recHits, 
						       ...
						       const int eventIsMC) const
{
  ...
  const float scale = correctionRetriever_.scaleCorr(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC);  
  const float smear = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 0., 0.);

  // This always to be carefully thought
  float scale_up = scale + correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 7);
  float scale_dn = scale - correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 7);
  float resol_up  = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 1., 0.);
  float resol_dn  = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, -1., 0.);

  float scale_stat_up = scale + correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 1);
  float scale_stat_dn = scale - correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 1);
  float scale_syst_up = scale + correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 2);
  float scale_syst_dn = scale - correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 2);
  float scale_gain_up = scale + correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 4);
  float scale_gain_dn = scale - correctionRetriever_.scaleCorrUncert(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 4);
  float resol_rho_up  = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 1., 0.);
  float resol_rho_dn  = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, -1., 0.);
  float resol_phi_up  = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 0., 1.);
  float resol_phi_dn  = correctionRetriever_.smearingSigma(runNumber, ele.isEB(), ele.full5x5_r9(), aeta, et, gainSeedSC, 0., -1.);
