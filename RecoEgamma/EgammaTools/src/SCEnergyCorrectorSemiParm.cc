...
void SCEnergyCorrectorSemiParm::setTokens(const edm::ParameterSet &iConfig, edm::ConsumesCollector &cc) {
  ...
  if(iConfig.existsAs<bool>("readFromFile")){
    ...
    if(readFromFile_){
      ...
      foresteb_ = dynamic_cast<const GBRForestD*>(fileEB->Get("EBCorrection"));
      forestsigmaeb_ = dynamic_cast<const GBRForestD*>(fileEB->Get("EBUncertainty"));
      edm::FileInPath filenameEE(iConfig.getParameter<std::string>("filenameEB")); 
      TFile* fileEE = TFile::Open(filenameEE.fullPath().c_str(),"READ");
      forestee_ = dynamic_cast<const GBRForestD*>(fileEE->Get("EECorrection"));
      forestsigmaee_ = dynamic_cast<const GBRForestD*>(fileEE->Get("EEUncertainty"));
      delete fileEB;
      delete fileEE;
    }
  }
