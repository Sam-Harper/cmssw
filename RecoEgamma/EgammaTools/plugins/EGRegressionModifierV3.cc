...
private:
  ...
  const edm::EventSetup* evtSetup_;
...
EGRegressionModifierV3::EGRegressionModifierV3(const edm::ParameterSet& conf) :
  ...
  rhoValue_(0.),
  rhoTag_(conf.getParameter<edm::InputTag>("rhoTag")),
...
void EGRegressionModifierV3::setConsumes(edm::ConsumesCollector& cc) 
{
  rhoToken_ = cc.consumes<double>(rhoTag_);
}
...
void EGRegressionModifierV3::getSeedCrysCoord(const reco::CaloCluster& clus,int& iEtaOrX,int& iPhiOrY)const
{
  ...
  if(useClosestToCentreSeedCrysDef_){
    ...
    if(isEB) ecalLocal.localCoordsEB(clus,*evtSetup_,dummy,dummy,iEtaOrX,iPhiOrY,dummy,dummy);
    else ecalLocal.localCoordsEE(clus,*evtSetup_,dummy,dummy,iEtaOrX,iPhiOrY,dummy,dummy);
  }else{
    if(isEB){
