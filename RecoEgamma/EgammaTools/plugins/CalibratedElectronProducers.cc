#ifndef RecoEgamma_EgammaTools_CalibratedElectronProducerRun2_h
#define RecoEgamma_EgammaTools_CalibratedElectronProducerRun2_h
...
CalibratedElectronProducerT<T>::CalibratedElectronProducerT( const edm::ParameterSet & conf ) :
  ...
  epCombinationTool_(),
  energyCorrector_(epCombinationTool_, conf.getParameter<bool>("isMC"), conf.getParameter<bool>("isSynchronization"), conf.getParameter<std::string>("correctionFile")),
...
CalibratedElectronProducerT<T>::produce( edm::Event & iEvent, const edm::EventSetup & iSetup ) 
{
  ...
  for (auto&& forestName : gbrForestName_) {
